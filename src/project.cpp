...
void Project::DoLoadAudio(agi::fs::path const& path, bool quiet) {
	...
	try {
		try {
			...
			audio_provider = GetAudioProvider(path, *context->path, NULL);
		}
...
bool Project::DoLoadVideo(agi::fs::path const& path) {
	try {
		...
		video_provider = agi::make_unique<AsyncVideoProvider>(path, old_matrix, context->videoController.get(), (DialogProgress*)NULL);
	}
	catch (agi::UserCancelException const&) { return false; }
